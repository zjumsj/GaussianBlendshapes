...
def quaternion_from_twovectors(a, b):
    '''
    ...
    # TODO, can not solve dot(a^,b^)==-1 case
    # where a^ = normalize(a), b^ = normalize(b)
    '''
    n = torch.cross(a,b,dim=-1)
    w = torch.sqrt((a * a).sum(-1) * (b * b).sum(-1)) + (a * b).sum(-1) # Nx3 -> N
    quat = torch.concat([w.unsqueeze(-1), n], dim=-1)
    quat = F.normalize(quat, dim=-1)
    return quat

def quaternion_avg(quaternions, weight = None, approx = False):
    """

    https://math.stackexchange.com/questions/61146/averaging-quaternions/3435296#3435296

    Args:
        quaternions: (N,4), w,x,y,z; suppose the quaternions are normalized
        weight: (N)
        approx:  whether to use approx algorithm
    ...
    """
    if weight is None:
        weight = torch.ones_like(quaternions[...,0])

    if approx:
        flag = torch.sum(quaternions[[0]] * quaternions, dim=-1) < 0.0
        flag = 1. - 2. * flag.float()
        weight = weight * flag
        output = torch.sum(quaternions * weight.unsqueeze(-1),dim=0)
        output = F.normalize(output, dim=-1)
    else:
        quaternions_ = quaternions.unsqueeze(-2) # Nx1x4
        # Nx4x1 @ Nx1x4 -> Nx4x4
        output = torch.bmm(quaternions.unsqueeze(-1), quaternions_) * weight.unsqueeze(-1).unsqueeze(-1)
        output = torch.sum(output,dim=0)
        output = torch.linalg.eigh(output)
        eigenvalues = output.eigenvalues
        eigenvectors = output.eigenvectors
        #print('eigenvalues', eigenvalues)
        #print('eigenvectors', eigenvectors)
        id = torch.argmax(eigenvalues,dim=0)
        #print(id)
        #output = eigenvectors[id,...]
        output = eigenvectors[...,id]

    return output


# NOTE: maybe not numerical stable, see pytorch3d's implementation
@torch.no_grad()
def log_map_SO3xR3(transforms: torch.Tensor) -> torch.Tensor:
    #  Tensor Nx3x4 -> Nx6
    eps = 1e-4
    #eps = 1e-6
    R = transforms[:,:,:3]
    ...
    phi = torch.acos(torch.clamp(phi_cos,-1.0,1.0))
    ...
    y = log_rot_hat[:,0,2]
    z = log_rot_hat[:,1,0]
    log_rot = torch.stack([x,y,z],dim=1)
    return torch.cat((transforms[:,:3,3],log_rot),dim=-1)
